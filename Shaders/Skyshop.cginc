...
/////////////////////////////////////////////////////////////////////////////////
/// @file Skyshop.cginc
/// @brief Skyshop ambient lighting implementation.
/////////////////////////////////////////////////////////////////////////////////

#ifndef ALLOY_SKYSHOP_CGINC
#define ALLOY_SKYSHOP_CGINC

#include "Assets/Alloy/Shaders/Core.cginc"
	
#if defined(_ENVIRONMENTMAPMODE_SKYSHOP)
	//-------------------------------------------------------------------------------
	// Skyshop functions
	// Code used with permission from Marmoset for the purpose of utilizing content
	// generated via Skyshop.
	//-------------------------------------------------------------------------------
	#if MARMO_BOX_PROJECTION_ON	
		#define MARMO_BOX_PROJECTION
	#endif

	#if MARMO_SKY_BLEND_ON || MARMO_GLOBAL_BLEND_ON
...
/// Skyshop ambient SH/Cube Spherical/Box lighting
...
{ 
	...
	half3 diffuse = blendedDiffuseIBL(normalWs) * exposureIBL.x;
	
	//SPECULAR IBL
	half3 specular;
	half glossLod = glossLOD(o.Specular, 8.0h); // 8 is the maximum skyshop uses.
	float3 skyR = reflectionWs;
	...
	specular = glossCubeLookup(_SpecCubeIBL, skyR, glossLod);
	
	#ifdef MARMO_SKY_BLEND		
		half3 specIBL1 = glossCubeLookup(_SpecCubeIBL1, skyR1, glossLod);
		specular = lerp(specIBL1, specular, skyWeight);
	#endif
	
	specular *= exposureIBL.y;
	...
	half3 fdv = specular * AlloyLazarovAmbientBrdf(specularColor, o.Specular, ndv);
		...
		half3 diffuseAmbient = diffuse + 2.0h * ShadeSH9(float4(normalWs, 1.0f));

		o.Emission += occludedAlbedo * diffuseAmbient +
				     lerp(specularColorAo * diffuse, fdv, so);
	#elif !defined(ALLOY_NO_LIGHTMAP_BRDF)
		// These contributions are not affected by ambient lighting.
		o.Emission += occludedAlbedo * diffuse +
				     lerp(specularColorAo * diffuse, fdv, so);
				     
		// Ambient BRDF data for use in lightmapping callbacks.
		o.AmbientBrdf = occludedAlbedo * exposureIBL.w;
	#endif

	// Skyshop Camera Exposure
	o.Albedo *= exposureIBL.w;
	o.SpecularOcclusion *= exposureIBL.w;
	o.Emission *= exposureIBL.w;
#endif
}

#endif // ALLOY_SKYSHOP_CGINC
