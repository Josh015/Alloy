...
half4 CalculateLight (v2f i)
{
	...
	// Alloy Code Begin
	//---------------------------------------------------------------------------
	half3 Vn = normalize(wpos - _WorldSpaceCameraPos);
	half3 Hn = normalize(lightDir - Vn);
	half ndl = max(0.0h, dot(normal, lightDir));
	half ndh = max(0.0h, dot(normal, Hn));
	half ldh = max(0.0h, dot(lightDir, Hn));
	half gloss = nspec.a;
	half d = AlloyBlinnPhongDistribution(gloss, ndh);
	...
	// Apply luminance of color to specular lighting to allow color recovery.
	// http://www.realtimerendering.com/blog/deferred-lighting-approaches/
  	result.a = AlloyLuminance(_LightColor.rgb) * d * v; 
	
	// Dual-Lightmap fade.
	float fade = fadeDist * unity_LightmapFade.z + unity_LightmapFade.w;
	
	// Apply punctual lighting equation and dual-lightmap fade.
	return result * (ndl * atten * saturate(1.0h - fade));
	//---------------------------------------------------------------------------
	// Alloy Code End
	//---------------------------------------------------------------------------
}
